...
const currentImage = computed(() => props.listing.images[currentImageIndex.value] || null);

const hasPrevious = computed(() => currentImageIndex.value > 0);
const hasNext = computed(() => currentImageIndex.value < props.listing.images.length - 1)
...
<template>
    <div class="flex flex-col-reverse md:grid md:grid-cols-12 gap-4">
        <Box class="md:col-span-7 flex flex-col items-center w-full">
            <div v-if="listing.images.length" class="relative w-full">
                <div class="relative cursor-pointer group" @click="openModal">
                    <img :src="currentImage.src" class="w-full h-auto transition-opacity duration-300"
                        :class="{ 'group-hover:opacity-75': listing.images.length > 0 }" />
                    <div
                        class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <span class="text-white text-xl">Click to zoom</span>
                    </div>
                </div>

                <button @click="previous" v-if="hasPrevious"
                    class="absolute left-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full">
                    &larr;
                </button>
                <button @click="next" v-if="hasNext"
            ...
            <!-- Image indicators -->
            <div v-if="listing.images.length > 1" class="flex justify-center mt-4 space-x-2">
                <button v-for="(image, index) in listing.images" :key="image.id" @click="currentImageIndex = index"
                    class="w-3 h-3 rounded-full transition-all duration-300 focus:outline-none"
                    :class="index === currentImageIndex ? 'bg-blue-500' : 'bg-gray-300 hover:bg-gray-400'"></button>
            </div>

            <!-- Thumbnail previews -->
            <div v-if="listing.images.length > 1" class="flex justify-center mt-4 space-x-2">
                <img v-for="(image, index) in listing.images" :key="image.id" :src="image.src"
                    @click="currentImageIndex = index"
                    class="w-16 h-16 object-cover cursor-pointer transition-all duration-300"
                    :class="index === currentImageIndex ? 'border-2 border-blue-500 scale-110' : 'opacity-70 hover:opacity-100'" />
            </div>
        </Box>
        <div class="md:col-span-5 flex flex-col gap-4">
            ...
            <Box>
                ...
                <div>
                    ...
                    <input v-model.number="interestRate" type="range" min="0.25" max="30" step="0.1"
    ...
    <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-95"
        ...
        <div v-if="isModalOpen" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50"
            ...
            <div class="max-w-4xl max-h-full relative" @click.stop>
                ...
                <img :src="currentImage.src" class="max-w-full max-h-full object-contain" @click.stop>
            </div>
        </div>
    </Transition>
</template>
